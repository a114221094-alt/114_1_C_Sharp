...
namespace Q2
{
    public partial class Form1 : Form
    {
        ...
        {
            // populate services with ServiceItem to show Chinese
            checkedListBoxServices.Items.Clear();
            foreach (ServiceType st in Enum.GetValues(typeof(ServiceType)))
            {
                checkedListBoxServices.Items.Add(new ServiceItem(st));
            }
        ...
        {
            var selected = checkedListBoxServices.CheckedItems.Cast<ServiceItem>().ToList();
            decimal serviceFee = 0m;
            var displayNames = new List<string>();
            foreach (var item in selected)
            {
                serviceFee += ServiceCalculator.GetBasePrice(item.Type);
